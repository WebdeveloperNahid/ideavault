...
export default function IdeaCard({ idea, showActions = false, onEdit, onDelete }) {
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden flex-shrink-0">
        ...
        <div className={`absolute inset-0 transition-all duration-300 ${
          hovered ? "bg-gradient-to-b from-sky-400/10 to-black/40" : "bg-gradient-to-b from-transparent via-transparent to-black/25"
        }`} />
        <span className={`absolute top-3 left-3 flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-semibold ${cat.bg} ${cat.text}`}>
      ...
      <div className="flex flex-col flex-1 gap-2.5 p-5">
        <h3 className={`text-[17px] font-bold leading-snug tracking-tight transition-colors duration-200 ${hovered ? "text-sky-500" : "text-slate-900"}`}>
          {idea.ideaTitle}
        </h3>
        ...
        <div className="bg-sky-50 border border-sky-100 rounded-xl px-3.5 py-2.5">
          <p className="text-[10.5px] font-semibold text-sky-500 uppercase tracking-widest mb-0.5">Problem</p>
          <p className="text-[12.5px] text-slate-600 leading-relaxed line-clamp-2">{idea.problemStatement}</p>
        </div>

        {/* ✅ Solution — নতুন যোগ হয়েছে */}
        {idea.proposedSolution && (
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-3.5 py-2.5">
            <p className="text-[10.5px] font-semibold text-emerald-500 uppercase tracking-widest mb-0.5">Solution</p>
            <p className="text-[12.5px] text-slate-600 leading-relaxed line-clamp-2">{idea.proposedSolution}</p>
          </div>
        )}
